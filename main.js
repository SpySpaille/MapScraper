...
async function VTFfromVMT(gamepath, sourcePath, outputDir) {
      try {
            ...
            for (const texture of textures) {
                  const sourcePath = path.join(gamepath, 'materials', texture.replace('.vtf', '') + '.vtf')
                  const destPath = path.join(outputDir, 'materials', String(texture).toLowerCase().replace('.vtf', '') + '.vtf')
